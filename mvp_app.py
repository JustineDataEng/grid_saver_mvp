...
sense_triggers     = int(df_view['sense_triggered'].sum()) if 'sense_triggered' in df_view.columns else 0
predict_triggers   = int(df_view['predict_triggered'].sum())
spa_actions        = int(df_view['spa_action_triggered'].sum()) if 'spa_action_triggered' in df_view.columns else 0
...
cards = [
    ...
    (col3, f"{vulnerable_pct:.1f}%",    "of period at risk",  "Vulnerability Rate",     "#4A9EFF"),
    (col4, f"{sense_triggers}",         "hours",              "Sense Triggers",         "#4A9EFF"),
    (col5, f"{predict_triggers}",       "hours",              "Predict Triggers",       "#9B59B6"),
    (col6, f"{spa_actions}",            "events",             "SPA Actions",            "#F39C12"),
    (col7, f"{current_prob:.2f}",       "probability",        "24hr Risk Projection (Temporal Pattern)",   "#9B59B6"),
]
...
# Shows before and after intervention plot + SPA action status
# ============================================================
...
spa_count_view = int(df_view['spa_action_triggered'].sum()) if 'spa_action_triggered' in df_view.columns else 0
sense_count_view   = int(df_view['sense_triggered'].sum()) if 'sense_triggered' in df_view.columns else 0
predict_count_view = int(df_view['predict_triggered'].sum())

col_sim1, col_sim2, col_sim3 = st.columns(3)
sim_cards = [
    (col_sim1, f"{sense_count_view}",   "hours triggered", "Sense Layer",   "#4A9EFF"),
    (col_sim2, f"{predict_count_view}", "hours triggered", "Predict Layer", "#9B59B6"),
    (col_sim3, f"{spa_action_view}" if (spa_action_view := spa_count_view) else "0",
               "events confirmed", "SPA Actions (Dual-Confirmed)", "#F39C12"),
]
...
if 'grid_saver_reduction_kw' in df_view.columns:
    original_score    = df_view['vulnerability_score']
    simulated_score   = (df_view['vulnerability_score'] - df_view['grid_saver_reduction_kw'] / 500).clip(lower=0)
...
fig_sim.update_layout(
    ...
    font=dict(color='white'),
    title=dict(
        text='Grid Vulnerability Before and After Grid Saver Intervention',
